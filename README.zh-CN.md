...
DevLoop 通过能力检测适配不同 AI 编码宿主，不依赖单一宿主特性。

| 能力 | 启用场景 | 缺失时降级 |
|------|----------|------------|
| 子代理/并行工作线程 | 并行 code-review、并行 verify | 串行执行相同步骤 |
| 后台任务 | 非阻塞 verify + review pipeline | 显式阶段门控同步流水线 |
| 自动 skill 触发 | `/develop` 在 intake 阶段自动调用 | 用户手动调用 `/develop` |
| 工作区文件协作 | 同一仓库多代理共享状态 | 锁 + 单写入者状态更新 |

**支持的宿主（能力感知）：**

| 宿主 | 状态 | 能力说明 |
|------|------|----------|
| Claude Code | ✅ 完整 | 子代理、后台任务 |
| Codex | ✅ 完整 | 需能力检测 |
| OpenCode | ✅ 完整 | 需能力检测 |
| TRAE | ✅ 完整 | 需能力检测 |
| Cursor | ✅ 完整 | 需能力检测 |
...
**降级规则（宿主无关）：**
- 不支持子代理 → `code-review` 串行执行
- 不支持后台任务 → 禁用异步流水线阶段
- 不支持自动触发 → 用户手动调用 `/develop`
